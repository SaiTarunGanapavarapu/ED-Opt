...
function [params] = getParams()
    ...
    params.memWidth      = 0.024      ;      % Membrane Width  (m) 
    params.memLength     = 8          ;      % Membrane Length (m) 
    params.thicknessConc = 0.001      ;      % Thickness of concentrate channel (m)
    params.thicknessDil  = 0.001      ;      % Thickness of dilute      channel (m)  
    params.feedSplit     = 0.077      ;      % Feed split deciding flow to both channels
    params.voltCellPair  = 0.44       ;      % Cell-Pair voltage specification (V)
    params.voltTotal     = 221.82     ;      % Total voltage specification (V)
    params.currDens      = 160.77     ;      % Current Density (A/m2)
    params.curr          = 30.85      ;      % Current (A)
    ...
    % if params.feedSplit < 10^-5
    %     params.feedSplit = 10^-5;
    % end
    params.flowConcIn    = params.flowIn*params.feedSplit      /params.numCells ; 
    params.flowDilIn     = params.flowIn*(1 - params.feedSplit)/params.numCells ; 
